...
def get_scraped_urls():
    ...
    scraped_urls = set()
    if os.path.exists(config_fisheries_lv.OUTPUT_CSV):
        try:
            with open(config_fisheries_lv.OUTPUT_CSV, mode='r', encoding='utf-8-sig') as f:
...
def append_to_csv(row_dict):
    ...
    file_exists = os.path.isfile(config_fisheries_lv.OUTPUT_CSV)
    try:
        with open(config_fisheries_lv.OUTPUT_CSV, mode='a', encoding='utf-8-sig', newline='') as f:
...
PROGRESS_FILE = config_fisheries_lv.PROGRESS_FILE
            ...
            with open(config_fisheries_lv.OUTPUT_CSV, mode='r', encoding='utf-8-sig') as f:
                ...
                for row in reader:
                    ...
                    if loc_name and state:
                        pair = (loc_name.strip().lower(), state.strip().lower(), config_fisheries_lv.KEYWORDS[0].lower())
...
async def scroll_feed(page, max_scrolls=20):
    ...
    feed_selector = config_fisheries_lv.SELECTORS["results_container"]
...
async def extract_details(page, url, search_query, loc_info):
    ...
    if category:
        ...
        if not any(tag in cat_lower for tag in config_fisheries_lv.ALLOWED_CATEGORIES):
...
async def process_search(page, keyword, loc_info, scraped_urls):
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            await page.goto(search_url, timeout=config_fisheries_lv.TIMEOUT, wait_until="domcontentloaded")
    ...
    if listings_count > 0:
        ...
        try:
            ...
            addr_loc = page.locator(config_fisheries_lv.SELECTORS["address"])
            address = ""
            if await addr_loc.count() > 0:
                addr_label = await addr_loc.first.get_attribute("aria-label")
    ...
    for index, url in enumerate(urls):
        ...
        await asyncio.sleep(random.uniform(config_fisheries_lv.MIN_DELAY, config_fisheries_lv.MAX_DELAY))
...
async def main():
    print("="*60)
    print("      LATVIA FISHERIES & SEAFOOD GOOGLE MAPS SCRAPER")
    print("="*60)
    ...
    keywords = config_fisheries_lv.KEYWORDS
    ...
    print(f"[+] Output CSV path: {config_fisheries_lv.OUTPUT_CSV}\n")
    ...
    async with async_playwright() as p:
        ...
        for channel in ["chrome", "msedge", None]:
            try:
                ...
                launch_args = {
                    "headless": config_fisheries_lv.HEADLESS,
    ...
    print(f"[+] Results saved to: {config_fisheries_lv.OUTPUT_CSV}")
