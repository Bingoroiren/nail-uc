...
def get_scraped_urls():
    ...
    scraped_urls = set()
    if os.path.exists(config_tw.OUTPUT_CSV):
        try:
            with open(config_tw.OUTPUT_CSV, mode='r', encoding='utf-8-sig') as f:
...
def append_to_csv(row_dict):
    ...
    file_exists = os.path.isfile(config_tw.OUTPUT_CSV)
    try:
        with open(config_tw.OUTPUT_CSV, mode='a', encoding='utf-8-sig', newline='') as f:
...
PROGRESS_FILE = os.path.join(os.path.dirname(config_tw.OUTPUT_CSV), "scraping_progress_tw.json")
...
async def scroll_feed(page, max_scrolls=20):
    ...
    feed_selector = config_tw.SELECTORS["results_container"]
...
async def extract_details(page, url, search_query, loc_info):
    ...
    # Filter category tag
    ALLOWED_CATEGORIES = set(config_tw.KEYWORDS)
    if not category or category.strip() not in ALLOWED_CATEGORIES:
        print(f"    [-] Skipping: Category '{category}' is not in construction tags.")
        return None
...
async def process_search(page, keyword, loc_info, scraped_urls):
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            await page.goto(search_url, timeout=config_tw.TIMEOUT, wait_until="domcontentloaded")
    ...
    link_selector = config_tw.SELECTORS["listing_link"]
    ...
    if listings_count > 0:
        ...
        try:
            ...
            name_matched = False
            for _ in range(15):
                h1_locator = page.locator(config_tw.SELECTORS["business_name"])
            ...
            addr_loc = page.locator(config_tw.SELECTORS["address"])
            address = ""
            if await addr_loc.count() > 0:
                addr_label = await addr_loc.first.get_attribute("aria-label")
                address = addr_label
    ...
    for index, url in enumerate(urls):
        ...
        await asyncio.sleep(random.uniform(config_tw.MIN_DELAY, config_tw.MAX_DELAY))
...
async def main():
    print("="*60)
    print("      TAIWAN CONSTRUCTION COMPANY GOOGLE MAPS SCRAPER")
    print("="*60)
    ...
    keywords = config_tw.KEYWORDS
    
    print(f"[+] Total target locations: {len(locs)}")
    print(f"[+] Search keywords: {keywords}")
    print(f"[+] Output CSV path: {config_tw.OUTPUT_CSV}\n")
    ...
    async with async_playwright() as p:
        ...
        for channel in ["chrome", "msedge", None]:
            try:
                ...
                launch_args = {
                    "headless": config_tw.HEADLESS,
                    "slow_mo": config_tw.SLOW_MO,
                    "args": [
    ...
    print(f"[+] Results saved to: {config_tw.OUTPUT_CSV}")
