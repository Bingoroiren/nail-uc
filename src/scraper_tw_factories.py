...
import config_tw_factories
import locations_tw

# Set console output encoding to UTF-8 to prevent print errors with Chinese or special chars
...
def extract_place_id(url):
    if not url:
        return ""
    # Match hex Place ID pair in the URL, e.g. 1s0x3442ac6b58240f39:0x8544e3dbff069c9
...
def get_scraped_urls():
    ...
    scraped_urls = set()
    if os.path.exists(config_tw_factories.OUTPUT_CSV):
        try:
            with open(config_tw_factories.OUTPUT_CSV, mode='r', encoding='utf-8-sig') as f:
...
def is_taiwan_address(address):
    """Verifies if an address is in Taiwan by checking for country name, cities, or common patterns."""
    if not address:
        return False
    addr_lower = address.lower()
    if "taiwan" in addr_lower or "tai wan" in addr_lower or "台灣" in addr_lower or "臺灣" in addr_lower or "中華民國" in addr_lower or "r.o.c." in addr_lower:
        return True
    
    # Taiwan counties and cities
    tw_cities = [
        "台北", "臺北", "新北", "桃園", "台中", "臺中", "台南", "臺南", "高雄", "基隆", "新竹", "嘉義", 
        "苗栗", "彰化", "南投", "雲林", "屏東", "宜蘭", "花蓮", "台東", "臺東", "澎湖", "金門", "連江",
        "taipei", "taoyuan", "taichung", "tainan", "kaohsiung", "keelung", "hsinchu", "chiayi",
        "miaoli", "changhua", "nantou", "yunlin", "pingtung", "yilan", "hualien", "taitung", "penghu", "kinmen", "matsu"
    ]
    if any(city in addr_lower for city in tw_cities):
        return True
...
def append_to_csv(row_dict):
    ...
    file_exists = os.path.isfile(config_tw_factories.OUTPUT_CSV)
    try:
        with open(config_tw_factories.OUTPUT_CSV, mode='a', encoding='utf-8-sig', newline='') as f:
...
PROGRESS_FILE = os.path.join(os.path.dirname(config_tw_factories.OUTPUT_CSV), "scraping_progress_tw_factories.json")
        ...
        try:
            ...
            with open(config_tw_factories.OUTPUT_CSV, mode='r', encoding='utf-8-sig') as f:
                ...
                for row in reader:
                    ...
                    # Extract raw keyword if possible
                    raw_kw = ""
                    if kw:
                        for original_kw in config_tw_factories.KEYWORDS:
                            if original_kw in kw:
                                raw_kw = original_kw
                                break
                    if not raw_kw:
                        raw_kw = config_tw_factories.KEYWORDS[0]
...
async def bypass_consent_screen(page):
    ...
    try:
        consent_buttons = page.locator('button:has-text("Accept all"), button:has-text("Agree"), button:has-text("I agree"), button:has-text("Accept"), button:has-text("全部接受"), button:has-text("我同意"), button:has-text("同意"), button:has-text("接受")')
...
async def scroll_feed(page, max_scrolls=20):
    ...
    feed_selector = config_tw_factories.SELECTORS["results_container"]
    ...
    while scrolls < max_scrolls:
        ...
        if any(x in inner_text.lower() for x in ["reached the end of the list", "已到清單結尾", "已到底部", "沒有更多結果"]):
...
def parse_taiwan_address(address):
    """Extracts city/district and postcode from a standard Taiwan address."""
    if not address:
        return None, None, None
        
    addr = address.replace("Taiwan", "").replace("taiwan", "").replace("台灣", "").replace("臺灣", "").strip()
    
    # Try to find zipcode (3 to 6 digits)
    postcode = ""
    postcode_match = re.search(r'\b\d{3,6}\b', address)
    if postcode_match:
        postcode = postcode_match.group(0)
        
    # Find City (e.g. 台北市, 新北市, etc.)
    city = ""
    city_match = re.search(r'(台北市|臺北市|新北市|桃園市|台中市|臺中市|台南市|臺南市|高雄市|基隆市|新竹市|新竹縣|苗栗縣|彰化縣|南投縣|雲林縣|嘉義市|嘉義縣|屏東縣|宜蘭縣|花蓮縣|台東縣|臺東縣|澎湖縣|金門縣|連江縣)', address)
    if city_match:
        city = city_match.group(0)
    else:
        city_match_en = re.search(r'\b(taipei|new taipei|taoyuan|taichung|tainan|kaohsiung|keelung|hsinchu|chiayi|miaoli|changhua|nantou|yunlin|pingtung|yilan|hualien|taitung|penghu|kinmen|matsu)\b', address, flags=re.IGNORECASE)
        if city_match_en:
            city = city_match_en.group(0).title()
            
    # Find District/Township (e.g. 中正區, 板橋區, etc.)
    district = ""
    dist_match = re.search(r'([^\s,]{2,5}(區|鄉|鎮|市))', addr)
    if dist_match:
        district = dist_match.group(0)
        
    state = city if city else "TW"
...
async def extract_details(page, url, search_query, loc_info):
    ...
    sel = config_tw_factories.SELECTORS
    ...
    if await addr_loc.count() > 0:
        ...
        if addr_label:
            address = addr_label
            for prefix in ["Address:", "Adrese:", "地址:", "地址："]:
    ...
    # Validate Address is in Taiwan
    if not is_taiwan_address(address):
        print(f"    [-] Skipping: Address '{address}' is not in Taiwan.")
        return None
    ...
    if coords:
        ...
        # Taiwan bounding box check
        if not (21.8 < actual_lat < 26.5) or not (119.3 < actual_lng < 122.5):
            print(f"    [-] Skipping: Coordinates ({actual_lat}, {actual_lng}) are outside Taiwan.")
            return None
            
    # Parse Suburb and State
    actual_suburb, actual_state, postcode = parse_taiwan_address(address)
    if not actual_state:
    ...
    if await rating_loc.count() > 0:
        ...
        span_reviews = rating_loc.first.locator('span[aria-label*="則評論"], span[aria-label*="review"], span[aria-label*="reviews"]')
    ...
    try:
        closed_loc = page.locator('span:has-text("Permanently closed"), span:has-text("永久停業"), span:has-text("永久關閉")')
    ...
    # Filter category tag (includes 公司辦公室)
    ALLOWED_CATEGORIES = set(config_tw_factories.ALLOWED_CATEGORIES)
    if not category or category.strip() not in ALLOWED_CATEGORIES:
        print(f"    [-] Skipping: Category '{category}' is not in factories tags.")
        return None
...
async def process_search(page, keyword, loc_info, scraped_urls):
    ...
    search_query = f"{keyword} in {loc_info['name']}, Taiwan"
    print(f"\n[+] Searching: '{search_query}'")
    
    # Construct coordinate search URL using Taiwan locale (Traditional Chinese)
    query_encoded = urllib.parse.quote_plus(keyword)
    search_url = f"https://www.google.com/maps/search/{query_encoded}/@{loc_info['lat']},{loc_info['lng']},{loc_info['zoom']}z?hl=zh-TW"
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            await page.goto(search_url, timeout=config_tw_factories.TIMEOUT, wait_until="domcontentloaded")
    ...
    link_selector = config_tw_factories.SELECTORS["listing_link"]
    listings_count = await page.locator(link_selector).count()
    
    # Geofence check on first listing
    if listings_count > 0:
        ...
        try:
            ...
            name_matched = False
            for _ in range(15):
                h1_locator = page.locator(config_tw_factories.SELECTORS["business_name"])
            ...
            addr_loc = page.locator(config_tw_factories.SELECTORS["address"])
            address = ""
            if await addr_loc.count() > 0:
                addr_label = await addr_loc.first.get_attribute("aria-label")
                address = addr_label
            ...
            if address:
                if not is_taiwan_address(address):
                    print(f"[-] First listing is outside Taiwan ('{address}'). Skipping location.")
                    return
            else:
    ...
    for index, url in enumerate(urls):
        ...
        try:
            ...
            if record:
                ...
                print("    [-] Failed to parse/validate details card.")
        except Exception as parse_err:
            print(f"    [-] Exception during detail extraction: {parse_err}")
                
        await asyncio.sleep(random.uniform(config_tw_factories.MIN_DELAY, config_tw_factories.MAX_DELAY))
...
async def main():
    print("="*60)
    print("      TAIWAN FACTORY GOOGLE MAPS SCRAPER")
    print("="*60)
    ...
    locs = locations_tw.get_locations()
    keywords = config_tw_factories.KEYWORDS
    
    print(f"[+] Total target locations: {len(locs)}")
    print(f"[+] Search keywords: {keywords}")
    print(f"[+] Output CSV path: {config_tw_factories.OUTPUT_CSV}\n")
    ...
    async with async_playwright() as p:
        ...
        for channel in ["chrome", "msedge", None]:
            try:
                ...
                launch_args = {
                    "headless": config_tw_factories.HEADLESS,
                    "slow_mo": config_tw_factories.SLOW_MO,
                    "args": [
        ...
        context = await browser.new_context(
            ...
            geolocation={"latitude": 25.0375, "longitude": 121.5625},  # Default to Taipei coordinates
    ...
    print(f"[+] Results saved to: {config_tw_factories.OUTPUT_CSV}")
