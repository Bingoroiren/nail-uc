...
def get_scraped_urls():
    """Loads already scraped business URLs from the CSV file to avoid duplicates."""
    scraped_urls = set()
...
PROGRESS_FILE = os.path.join(os.path.dirname(config.OUTPUT_CSV), "scraping_progress.json")
            ...
            with open(config.OUTPUT_CSV, mode='r', encoding='utf-8-sig') as f:
                ...
                for row in reader:
                    ...
                    if loc_name and state:
                        pair = (loc_name.strip().lower(), state.strip().lower(), "nail salon")
                        if pair not in completed:
                            completed.add(pair)
                            completed_list.append([loc_name.strip(), state.strip(), "nail salon"])
...
async def scroll_feed(page, max_scrolls=20):
    ...
    feed_selector = config.SELECTORS["results_container"]
    
    # Wait to see if results container appears
    try:
        await page.wait_for_selector(feed_selector, timeout=8000)
    except Exception as e:
        # If it doesn't appear, maybe there is only one result or no results
        # Print warning to diagnose
        # print(f"    [-] Results container '{feed_selector}' did not load: {e}")
        return
...
async def process_search(page, keyword, loc_info, scraped_urls):
    ...
    link_selector = config.SELECTORS["listing_link"]
    listings_count = await page.locator(link_selector).count()
    
    # Check if the first result is in Australia before scrolling (to fail fast if Google fell back to Vietnam/Japan)
    if listings_count > 0:
        ...
        try:
            ...
            for _ in range(15): # Try for 3 seconds (15 * 200ms)
                h1_locator = page.locator(config.SELECTORS["business_name"])
            ...
            addr_loc = page.locator(config.SELECTORS["address"])
            address = ""
            if await addr_loc.count() > 0:
                addr_label = await addr_loc.first.get_attribute("aria-label")
    ...
    # Otherwise, it's a list. Scroll the results list.
    ...
    for index, url in enumerate(urls):
        ...
        # Try to click on the element in the list first (faster)
        clicked = False
        try:
            item_locator = page.locator(f'a.hfpxzc[href="{url}"]')
            if await item_locator.count() > 0:
                # Get the business name from the aria-label of the card
                ...
                # Wait for detail card to update to match expected name
                if expected_name:
                    name_matched = False
                    for _ in range(15): # Try for 3 seconds (15 * 200ms)
                        h1_locator = page.locator(config.SELECTORS["business_name"])
            ...
            # If clicking in-list fails, fallback to direct page navigation
            pass
...
async def main():
    print("="*60)
    print("      AUSTRALIA NAIL SALON GOOGLE MAPS SCRAPER")
    print("="*60)
    ...
    print(f"[+] Output CSV path: {config.OUTPUT_CSV}\n")
    ...
    async with async_playwright() as p:
        ...
        # Create a browser context with custom user agent and locale settings
        context = await browser.new_context(
            user_agent="Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36",
            locale="en-US",
            viewport={"width": 1280, "height": 800},
            geolocation={"latitude": -33.8688, "longitude": 151.2093},  # Default to Sydney, Australia coordinates
        ...
        try:
            for loc in locs:
                for kw in keywords:
                    ...
                    print(f"\n[Progress: {scan_index}/{total_scans}] Location: {loc['name']} ({loc['state']}) | Keyword: '{kw}'")
