...
def get_scraped_urls():
    ...
    scraped_urls = set()
    if os.path.exists(config_auto_au.OUTPUT_CSV):
        try:
            with open(config_auto_au.OUTPUT_CSV, mode='r', encoding='utf-8-sig') as f:
...
def append_to_csv(row_dict):
    ...
    file_exists = os.path.isfile(config_auto_au.OUTPUT_CSV)
    try:
        with open(config_auto_au.OUTPUT_CSV, mode='a', encoding='utf-8-sig', newline='') as f:
...
PROGRESS_FILE = config_auto_au.PROGRESS_FILE
...
async def scroll_feed(page, max_scrolls=20):
    ...
    feed_selector = config_auto_au.SELECTORS["results_container"]
...
async def extract_details(page, url, search_query, loc_info):
    ...
    # Strict filter: category must match one of the allowed auto/car tags
    if category:
        cat_lower = category.lower().strip()
        if not any(tag in cat_lower for tag in config_auto_au.ALLOWED_CATEGORIES):
            print(f"    [-] Skipping: Category '{category}' is not in allowed auto/car tags.")
            return None
    else:
        # If there is no category at all, skip it since we require strict category filtering
        print("    [-] Skipping: No category found on Google Maps details card.")
        return None
...
async def process_search(page, keyword, loc_info, scraped_urls):
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            await page.goto(search_url, timeout=config_auto_au.TIMEOUT, wait_until="domcontentloaded")
    ...
    link_selector = config_auto_au.SELECTORS["listing_link"]
    ...
    if listings_count > 0:
        ...
        try:
            ...
            for _ in range(15):
                h1_locator = page.locator(config_auto_au.SELECTORS["business_name"])
            ...
            addr_loc = page.locator(config_auto_au.SELECTORS["address"])
            address = ""
            if await addr_loc.count() > 0:
                addr_label = await addr_loc.first.get_attribute("aria-label")
    ...
    for index, url in enumerate(urls):
        ...
        await asyncio.sleep(random.uniform(config_auto_au.MIN_DELAY, config_auto_au.MAX_DELAY))
...
async def main():
    print("="*60)
    print("    AUSTRALIA AUTO REPAIR & MANUFACTURING GOOGLE MAPS SCRAPER")
    print("="*60)
    ...
    keywords = config_auto_au.KEYWORDS
    ...
    print(f"[+] Output CSV path: {config_auto_au.OUTPUT_CSV}\n")
    ...
    async with async_playwright() as p:
        ...
        for channel in ["chrome", "msedge", None]:
            try:
                ...
                launch_args = {
                    "headless": config_auto_au.HEADLESS,
                    "slow_mo": config_auto_au.SLOW_MO,
                    "args": [
    ...
    print(f"[+] Results saved to: {config_auto_au.OUTPUT_CSV}")
