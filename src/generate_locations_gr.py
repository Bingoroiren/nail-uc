...
def main():
    url = "https://simplemaps.com/static/data/country-cities/gr/gr.csv"
    print(f"[*] Downloading Greece locations database from: {url}")
    ...
    for row in reader:
        ...
        try:
            lat = float(lat_str)
            lng = float(lng_str)
            # Greece bounding box coordinates check (approximate Mainland + Islands)
            if not (34.0 < lat < 42.5) or not (19.0 < lng < 29.8):
                continue
            
            raw_locations.append({
                "state": state if state else "Greece",
                "name": locality,
                "lat": lat,
                "lng": lng
            })
    ...
    output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "locations_gr.py")
    print(f"[*] Writing {len(clustered_locations)} clustered locations to {output_path}...")
    
    try:
        with open(output_path, "w", encoding="utf-8") as f:
            f.write("# Generated automatically by generate_locations_gr.py\n")
            f.write("# Covers all populated locations in Greece using spatial clustering.\n\n")
            f.write("LOCATIONS = [\n")
        ...
        print("[SUCCESS] locations_gr.py has been successfully created!")
