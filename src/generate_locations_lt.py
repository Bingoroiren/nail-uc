...
def main():
    url = "https://simplemaps.com/static/data/country-cities/lt/lt.csv"
    print(f"[*] Downloading Lithuania locations database from: {url}")
    ...
    for row in reader:
        ...
        try:
            ...
            # Lithuania bounding box coordinates check
            if not (53.8 < lat < 56.6) or not (20.8 < lng < 27.0):
                continue
            
            raw_locations.append({
                "state": state if state else "Lithuania",
                "name": locality,
                "lat": lat,
                "lng": lng,
                "population": pop
    ...
    # Sort locations by population descending so major economic/industrial centers serve as cluster centers
    raw_locations.sort(key=lambda x: x["population"], reverse=True)
    ...
    # Save the generated locations into src/locations_lt.py
    output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "locations_lt.py")
    print(f"[*] Writing {len(clustered_locations)} clustered locations to {output_path}...")
    
    try:
        with open(output_path, "w", encoding="utf-8") as f:
            f.write("# -*- coding: utf-8 -*-\n")
            f.write("# Generated automatically by generate_locations_lt.py\n")
            f.write("# Covers all populated and industrial regions in Lithuania using spatial clustering at zoom 11.\n\n")
            f.write("LOCATIONS = [\n")
            for i, loc in enumerate(clustered_locations):
                covered_str = ", ".join(loc["covered_suburbs"][:6])
                if len(loc["covered_suburbs"]) > 6:
                    covered_str += f" and {len(loc['covered_suburbs']) - 6} more"
        ...
        print("[SUCCESS] locations_lt.py has been successfully generated for Lithuania!")
