...
def get_scraped_urls():
    ...
    scraped_urls = set()
    if os.path.exists(config_construction_au.OUTPUT_CSV):
        try:
            with open(config_construction_au.OUTPUT_CSV, mode='r', encoding='utf-8-sig') as f:
...
def is_australia_address(address):
    """Verifies if an address is in Australia."""
    if not address:
        return False
    ...
    if any(st in f" {addr_lower} " for st in au_states):
        return True
...
def append_to_csv(row_dict):
    ...
    file_exists = os.path.isfile(config_construction_au.OUTPUT_CSV)
    try:
        with open(config_construction_au.OUTPUT_CSV, mode='a', encoding='utf-8-sig', newline='') as f:
...
PROGRESS_FILE = config_construction_au.PROGRESS_FILE
            ...
            with open(config_construction_au.OUTPUT_CSV, mode='r', encoding='utf-8-sig') as f:
                ...
                for row in reader:
                    ...
                    if loc_name and state:
                        pair = (loc_name.strip().lower(), state.strip().lower(), config_construction_au.KEYWORDS[0].lower())
...
async def scroll_feed(page, max_scrolls=20):
    ...
    feed_selector = config_construction_au.SELECTORS["results_container"]
    ...
    while scrolls < max_scrolls:
        ...
        if "reached the end of the list" in inner_text.lower():
...
def parse_australian_address(address):
    ...
    addr = re.sub(r',\s*Australia\s*$', '', address, flags=re.IGNORECASE).strip()
    
    match = re.search(r',\s*([A-Z]{2,3})\s+(\d{4})$', addr, flags=re.IGNORECASE)
    if match:
        state = match.group(1).upper()
        postcode = match.group(2)
        remaining = addr[:match.start()].strip()
...
async def extract_details(page, url, search_query, loc_info):
    ...
    sel = config_construction_au.SELECTORS
    ...
    if not is_australia_address(address):
        print(f"    [-] Skipping: Address '{address}' is not in Australia.")
        return None
    ...
    if coords:
        ...
        # Australia Bounding box coordinates check
        if not (-44.0 < actual_lat < -10.0) or not (112.0 < actual_lng < 154.0):
            print(f"    [-] Skipping: Coordinates ({actual_lat}, {actual_lng}) are outside Australia.")
            return None
            
    actual_suburb, actual_state, postcode = parse_australian_address(address)
    if not actual_state:
        actual_suburb = loc_info["name"]
        actual_state = loc_info.get("state", "AU")
    ...
    if await rating_loc.count() > 0:
        ...
        span_reviews = rating_loc.first.locator('span[aria-label*="reviews"], span[aria-label*="review"]')
    ...
    # Strict filter: category must match one of the allowed construction tags
    if category:
        cat_lower = category.lower().strip()
        if not any(tag in cat_lower for tag in config_construction_au.ALLOWED_CATEGORIES):
            print(f"    [-] Skipping: Category '{category}' is not in allowed construction tags.")
            return None
...
async def process_search(page, keyword, loc_info, scraped_urls):
    ...
    search_query = f"{keyword} in {loc_info['name']}, Australia"
    print(f"\n[+] Searching: '{search_query}'")
    
    query_encoded = urllib.parse.quote_plus(keyword)
    zoom = 11  # Zoom map 10-11 as requested
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            await page.goto(search_url, timeout=config_construction_au.TIMEOUT, wait_until="domcontentloaded")
    ...
    link_selector = config_construction_au.SELECTORS["listing_link"]
    ...
    if listings_count > 0:
        ...
        try:
            ...
            for _ in range(15):
                h1_locator = page.locator(config_construction_au.SELECTORS["business_name"])
            ...
            addr_loc = page.locator(config_construction_au.SELECTORS["address"])
            address = ""
            if await addr_loc.count() > 0:
                addr_label = await addr_loc.first.get_attribute("aria-label")
            ...
            if address and not is_australia_address(address):
                print(f"[-] First listing is outside Australia ('{address}'). Skipping location.")
                return True
    ...
    for index, url in enumerate(urls):
        ...
        await asyncio.sleep(random.uniform(config_construction_au.MIN_DELAY, config_construction_au.MAX_DELAY))
...
async def main():
    print("="*60)
    print("      AUSTRALIA CONSTRUCTION COMPANY GOOGLE MAPS SCRAPER")
    print("="*60)
    ...
    locs = locations.get_locations()
    keywords = config_construction_au.KEYWORDS
    ...
    print(f"[+] Output CSV path: {config_construction_au.OUTPUT_CSV}\n")
    ...
    async with async_playwright() as p:
        ...
        for channel in ["chrome", "msedge", None]:
            try:
                ...
                launch_args = {
                    "headless": config_construction_au.HEADLESS,
                    "slow_mo": config_construction_au.SLOW_MO,
                    "args": [
    ...
    print(f"[+] Results saved to: {config_construction_au.OUTPUT_CSV}")
