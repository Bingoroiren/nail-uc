...
def main():
    url = "https://simplemaps.com/static/data/country-cities/lv/lv.csv"
    print(f"[*] Downloading Latvia locations database from: {url}")
    ...
    for row in reader:
        ...
        try:
            ...
            if not (55.0 < lat < 58.5) or not (20.5 < lng < 28.5):
                continue
            
            raw_locations.append({
                "state": state if state else "Latvia",
                "name": locality,
                "lat": lat,
                "lng": lng
            })
    ...
    print("[*] Performing spatial clustering to eliminate overlaps (radius ~7.0 km)...")
    ...
    # Save the generated locations back into src/locations_lv.py
    output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "locations_lv.py")
    print(f"[*] Writing {len(clustered_locations)} clustered locations to {output_path}...")
    
    try:
        with open(output_path, "w", encoding="utf-8") as f:
            f.write("# Generated automatically by generate_locations_lv.py\n")
            f.write("# Covers all populated locations in Latvia using spatial clustering.\n\n")
            f.write("LOCATIONS = [\n")
        ...
        print("[SUCCESS] locations_lv.py has been successfully updated with the Latvian dataset!")
