            ...
            with open(config_factory_ie.OUTPUT_CSV, mode='r', encoding='utf-8-sig') as f:
                ...
                for row in reader:
                    ...
                    if loc_name and state:
                        found_kw = ""
                        if search_query:
                            for kw in config_factory_ie.KEYWORDS:
                                if kw.lower() in search_query.lower():
                                    found_kw = kw
                                    break
                        if not found_kw:
                            found_kw = config_factory_ie.KEYWORDS[0]
...
async def scroll_feed(page, max_scrolls=25):
    ...
    feed_selector = config_factory_ie.SELECTORS["results_container"]
...
async def extract_details(page, url, search_query, loc_info):
    ...
    sel = config_factory_ie.SELECTORS
    ...
    # Strict filter: category must match one of the allowed Ireland factory tags
    if category:
        cat_lower = category.lower().strip()
        if not any(tag in cat_lower for tag in config_factory_ie.ALLOWED_CATEGORIES):
            print(f"    [-] Skipping: Category '{category}' is not in allowed Ireland factory tags.")
            return None
...
async def process_search(page, keyword, loc_info, scraped_urls):
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            await page.goto(search_url, timeout=config_factory_ie.TIMEOUT, wait_until="domcontentloaded")
    ...
    link_selector = config_factory_ie.SELECTORS["listing_link"]
    ...
    if listings_count > 0:
        ...
        try:
            ...
            for _ in range(15):
                h1_locator = page.locator(config_factory_ie.SELECTORS["business_name"])
            ...
            addr_loc = page.locator(config_factory_ie.SELECTORS["address"])
            address = ""
            if await addr_loc.count() > 0:
                addr_label = await addr_loc.first.get_attribute("aria-label")
...
async def main():
    print("="*60)
    print("        IRELAND ELECTRONICS & SEMICONDUCTOR FACTORIES MAPS SCRAPER")
    print("="*60)
    ...
    print(f"[+] Output CSV path: {config_factory_ie.OUTPUT_CSV}\n")
    ...
    async with async_playwright() as p:
        ...
        for channel in ["chrome", "msedge", None]:
            try:
                ...
                launch_args = {
                    "headless": config_factory_ie.HEADLESS,
                    "slow_mo": config_factory_ie.SLOW_MO,
                    "args": [
    ...
    print(f"[+] Results saved to: {config_factory_ie.OUTPUT_CSV}")
