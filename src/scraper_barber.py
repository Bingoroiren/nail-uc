...
def get_scraped_urls():
    ...
    scraped_urls = set()
    if os.path.exists(config_barber.OUTPUT_CSV):
        try:
            with open(config_barber.OUTPUT_CSV, mode='r', encoding='utf-8-sig') as f:
...
def is_australian_address(address):
    """Verifies if an address is in Australia by checking for country name or state + postcode pattern."""
    if not address:
        return False
    addr_lower = address.lower()
    if "australia" in addr_lower:
        return True
    # Match Australian state and 4-digit postcode (e.g. NSW 2263, VIC 3000)
    state_postcode_pattern = r'\b(nsw|vic|qld|wa|sa|tas|act|nt)\s+\d{4}\b'
    if re.search(state_postcode_pattern, addr_lower):
        return True
    return False

def append_to_csv(row_dict):
    """Appends a single scraped record to the output CSV file."""
    file_exists = os.path.isfile(config_barber.OUTPUT_CSV)
    try:
        with open(config_barber.OUTPUT_CSV, mode='a', encoding='utf-8-sig', newline='') as f:
...
PROGRESS_FILE = os.path.join(os.path.dirname(config_barber.OUTPUT_CSV), "scraping_progress_barber.json")
            ...
            with open(config_barber.OUTPUT_CSV, mode='r', encoding='utf-8-sig') as f:
                ...
                for row in reader:
                    ...
                    if loc_name and state:
                        pair = (loc_name.strip().lower(), state.strip().lower(), config_barber.KEYWORDS[0].lower())
                        if pair not in completed:
                            completed.add(pair)
                            completed_list.append([loc_name.strip(), state.strip(), config_barber.KEYWORDS[0]])
...
async def scroll_feed(page, max_scrolls=20):
    ...
    feed_selector = config_barber.SELECTORS["results_container"]
...
def parse_australian_address(address):
    """Extracts suburb and state from a standard Australian address."""
    if not address:
        return None, None, None
    addr = re.sub(r',\s*Australia\s*$', '', address, flags=re.IGNORECASE).strip()
    state_postcode_match = re.search(r'\b(NSW|VIC|QLD|WA|SA|TAS|ACT|NT)\s+(\d{4})$', addr, flags=re.IGNORECASE)
    if state_postcode_match:
        state = state_postcode_match.group(1).upper()
        postcode = state_postcode_match.group(2)
        remaining = addr[:state_postcode_match.start()].strip().rstrip(',')
...
async def extract_details(page, url, search_query, loc_info):
    ...
    if not is_australian_address(address):
    ...
    if coords:
        ...
        if not (-45.0 < actual_lat < -9.0) or not (110.0 < actual_lng < 155.0):
...
async def process_search(page, keyword, loc_info, scraped_urls):
    ...
    search_query = f"{keyword} in {loc_info['name']}, {loc_info['state']}, Australia"
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            await page.goto(search_url, timeout=config_barber.TIMEOUT, wait_until="domcontentloaded")
    ...
    link_selector = config_barber.SELECTORS["listing_link"]
    listings_count = await page.locator(link_selector).count()
    
    # Check if the first result is in Australia before scrolling (fail fast if fallback)
    if listings_count > 0:
        ...
        try:
            ...
            for _ in range(15): # Try for 3 seconds (15 * 200ms)
                h1_locator = page.locator(config_barber.SELECTORS["business_name"])
            ...
            addr_loc = page.locator(config_barber.SELECTORS["address"])
            address = ""
            if await addr_loc.count() > 0:
                addr_label = await addr_loc.first.get_attribute("aria-label")
    ...
    for index, url in enumerate(urls):
        ...
        await asyncio.sleep(random.uniform(config_barber.MIN_DELAY, config_barber.MAX_DELAY))
...
async def main():
    print("="*60)
    print("      AUSTRALIA BARBER SHOP GOOGLE MAPS SCRAPER")
    print("="*60)
    ...
    keywords = config_barber.KEYWORDS
    
    print(f"[+] Total target locations: {len(locs)}")
    print(f"[+] Search keywords: {keywords}")
    print(f"[+] Output CSV path: {config_barber.OUTPUT_CSV}\n")
    ...
    async with async_playwright() as p:
        ...
        for channel in ["chrome", "msedge", None]:
            try:
                ...
                launch_args = {
                    "headless": config_barber.HEADLESS,
                    "slow_mo": config_barber.SLOW_MO,
                    "args": [
        ...
        context = await browser.new_context(
            ...
            geolocation={"latitude": -33.86, "longitude": 151.21},  # Default to Sydney, Australia coordinates
    ...
    print(f"[+] Results saved to: {config_barber.OUTPUT_CSV}")
