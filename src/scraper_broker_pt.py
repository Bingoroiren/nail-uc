...
def get_scraped_urls():
    ...
    scraped_urls = set()
    if os.path.exists(config_broker_pt.OUTPUT_CSV):
        try:
            with open(config_broker_pt.OUTPUT_CSV, mode='r', encoding='utf-8-sig') as f:
...
def append_to_csv(row_dict):
    ...
    file_exists = os.path.isfile(config_broker_pt.OUTPUT_CSV)
    try:
        with open(config_broker_pt.OUTPUT_CSV, mode='a', encoding='utf-8-sig', newline='') as f:
...
PROGRESS_FILE = config_broker_pt.PROGRESS_FILE
            ...
            with open(config_broker_pt.OUTPUT_CSV, mode='r', encoding='utf-8-sig') as f:
                ...
                for row in reader:
                    ...
                    if loc_name and state:
                        found_kw = ""
                        if search_query:
                            for kw in config_broker_pt.KEYWORDS:
                                if kw.lower() in search_query.lower():
                                    found_kw = kw
                                    break
                        if not found_kw:
                            found_kw = config_broker_pt.KEYWORDS[0]
...
async def scroll_feed(page, max_scrolls=20):
    ...
    feed_selector = config_broker_pt.SELECTORS["results_container"]
...
async def extract_details(page, url, search_query, loc_info):
    ...
    sel = config_broker_pt.SELECTORS
    ...
    if await rating_loc.count() > 0:
        ...
        span_reviews = rating_loc.first.locator('span[aria-label*="avaliação"], span[aria-label*="avaliações"], span[aria-label*="review"], span[aria-label*="reviews"]')
    ...
    try:
        closed_loc = page.locator('span:has-text("Permanently closed"), span:has-text("Encerrado permanentemente")')
    ...
    if not any(tag in cat_lower for tag in config_broker_pt.ALLOWED_CATEGORIES):
        print(f"    [-] Skipping: Category '{category}' is not in Portugal recruitment tags.")
        return None
...
async def process_search(page, keyword, loc_info, scraped_urls):
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            await page.goto(search_url, timeout=config_broker_pt.TIMEOUT, wait_until="domcontentloaded")
    ...
    link_selector = config_broker_pt.SELECTORS["listing_link"]
    ...
    if listings_count > 0:
        ...
        try:
            ...
            name_matched = False
            for _ in range(15):
                h1_locator = page.locator(config_broker_pt.SELECTORS["business_name"])
            ...
            addr_loc = page.locator(config_broker_pt.SELECTORS["address"])
            address = ""
            if await addr_loc.count() > 0:
                addr_label = await addr_loc.first.get_attribute("aria-label")
                address = addr_label
                for prefix in ["Address:", "Adresa:", "Adrese:", "Endereço:", "Endereço：", "地址:", "地址："]:
    ...
    for index, url in enumerate(urls):
        ...
        await asyncio.sleep(random.uniform(config_broker_pt.MIN_DELAY, config_broker_pt.MAX_DELAY))
...
async def main():
    print("="*60)
    print("      PORTUGAL LABOR RECRUITMENT AGENCIES GOOGLE MAPS SCRAPER")
    print("="*60)
    ...
    keywords = config_broker_pt.KEYWORDS
    ...
    print(f"[+] Output CSV path: {config_broker_pt.OUTPUT_CSV}\n")
    ...
    async with async_playwright() as p:
        ...
        for channel in ["chrome", "msedge", None]:
            try:
                ...
                launch_args = {
                    "headless": config_broker_pt.HEADLESS,
                    "slow_mo": config_broker_pt.SLOW_MO,
                    "args": [
    ...
    print(f"[+] Results saved to: {config_broker_pt.OUTPUT_CSV}")
