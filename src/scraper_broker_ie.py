...
def get_scraped_urls():
    ...
    scraped_urls = set()
    if os.path.exists(config_broker_ie.OUTPUT_CSV):
        try:
            with open(config_broker_ie.OUTPUT_CSV, mode='r', encoding='utf-8-sig') as f:
...
def append_to_csv(row_dict):
    ...
    file_exists = os.path.isfile(config_broker_ie.OUTPUT_CSV)
    try:
        with open(config_broker_ie.OUTPUT_CSV, mode='a', encoding='utf-8-sig', newline='') as f:
...
PROGRESS_FILE = config_broker_ie.PROGRESS_FILE
            ...
            with open(config_broker_ie.OUTPUT_CSV, mode='r', encoding='utf-8-sig') as f:
                ...
                for row in reader:
                    ...
                    if loc_name and state:
                        pair = (loc_name.strip().lower(), state.strip().lower(), config_broker_ie.KEYWORDS[0].lower())
...
async def scrape_location_keyword(context, location, keyword, scraped_urls, completed_scans):
    ...
    page.set_default_timeout(config_broker_ie.TIMEOUT)
    ...
    try:
        ...
        try:
            await page.wait_for_selector(config_broker_ie.SELECTORS["results_container"], timeout=8000)
        except Exception:
            if await page.query_selector(config_broker_ie.SELECTORS["business_name"]):
                pass
            else:
        ...
        await scroll_feed(page, config_broker_ie.SELECTORS["results_container"])
        
        # Collect listing links
        links = await page.query_selector_all(config_broker_ie.SELECTORS["listing_link"])
        listing_urls = []
        for link in links:
        ...
        for i, listing_url in enumerate(listing_urls, 1):
            ...
            try:
                ...
                await page.wait_for_selector(config_broker_ie.SELECTORS["business_name"], timeout=5000)
                
                # Extract Name
                name_elem = await page.query_selector(config_broker_ie.SELECTORS["business_name"])
                ...
                # Extract Category
                category = ""
                cat_elems = await page.query_selector_all(config_broker_ie.SELECTORS["category"])
                ...
                # Extract Address
                address = ""
                addr_elem = await page.query_selector(config_broker_ie.SELECTORS["phone"] + ' ~ div, button[data-item-id^="address"]')
                if addr_elem:
                ...
                # Extract Website
                website = ""
                web_elem = await page.query_selector(config_broker_ie.SELECTORS["website"])
                if web_elem:
                    website = await web_elem.get_attribute("href") or ""
                    
                # Extract Phone
                phone = ""
                phone_elem = await page.query_selector(config_broker_ie.SELECTORS["phone"])
                if phone_elem:
                ...
                rating = ""
                rating_elem = await page.query_selector(config_broker_ie.SELECTORS["rating"])
                if rating_elem:
                    rating = await rating_elem.inner_text()
                    
                reviews_count = ""
                rev_elem = await page.query_selector(config_broker_ie.SELECTORS["reviews_count"])
                if rev_elem:
            ...
            await asyncio.sleep(random.uniform(config_broker_ie.MIN_DELAY, config_broker_ie.MAX_DELAY))
...
async def main():
    print("==================================================================")
    print("  IRELAND EMPLOYMENT AGENCIES & RECRUITERS GOOGLE MAPS SCRAPER  ")
    print("==================================================================")
    
    locations = locations_ie.get_locations()
    keywords = config_broker_ie.KEYWORDS
    ...
    async with async_playwright() as p:
        browser = await p.chromium.launch(headless=config_broker_ie.HEADLESS, slow_mo=config_broker_ie.SLOW_MO)
