...
def get_scraped_urls():
    ...
    scraped_urls = set()
    if os.path.exists(config_hotel_gr.OUTPUT_CSV):
        try:
            with open(config_hotel_gr.OUTPUT_CSV, mode='r', encoding='utf-8-sig') as f:
...
def append_to_csv(row_dict):
    ...
    file_exists = os.path.isfile(config_hotel_gr.OUTPUT_CSV)
    try:
        with open(config_hotel_gr.OUTPUT_CSV, mode='a', encoding='utf-8-sig', newline='') as f:
...
PROGRESS_FILE = config_hotel_gr.PROGRESS_FILE
            ...
            with open(config_hotel_gr.OUTPUT_CSV, mode='r', encoding='utf-8-sig') as f:
                ...
                for row in reader:
                    ...
                    if loc_name and state:
                        pair = (loc_name.strip().lower(), state.strip().lower(), config_hotel_gr.KEYWORDS[0].lower())
...
async def scroll_feed(page, max_scrolls=20):
    ...
    feed_selector = config_hotel_gr.SELECTORS["results_container"]
    ...
    while scrolls < max_scrolls:
        ...
        if "reached the end of the list" in inner_text.lower() or "τέλος της λίστας" in inner_text.lower() or "fim da lista" in inner_text.lower():
...
async def extract_details(page, url, search_query, loc_info):
    ...
    # Strict filter: category must match one of the allowed Greek hotel tags
    if category:
        cat_lower = category.lower().strip()
        if not any(tag in cat_lower for tag in config_hotel_gr.ALLOWED_CATEGORIES):
            print(f"    [-] Skipping: Category '{category}' is not in allowed Greek hotel tags.")
            return None
...
async def process_search(page, keyword, loc_info, scraped_urls):
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            await page.goto(search_url, timeout=config_hotel_gr.TIMEOUT, wait_until="domcontentloaded")
    ...
    link_selector = config_hotel_gr.SELECTORS["listing_link"]
    ...
    if listings_count > 0:
        ...
        try:
            ...
            for _ in range(15):
                h1_locator = page.locator(config_hotel_gr.SELECTORS["business_name"])
            ...
            addr_loc = page.locator(config_hotel_gr.SELECTORS["address"])
            address = ""
            if await addr_loc.count() > 0:
                addr_label = await addr_loc.first.get_attribute("aria-label")
    ...
    for index, url in enumerate(urls):
        ...
        await asyncio.sleep(random.uniform(config_hotel_gr.MIN_DELAY, config_hotel_gr.MAX_DELAY))
...
async def main():
    print("="*60)
    print("        GREECE HOTEL & ACCOMMODATION GOOGLE MAPS SCRAPER")
    print("="*60)
    ...
    keywords = config_hotel_gr.KEYWORDS
    ...
    print(f"[+] Output CSV path: {config_hotel_gr.OUTPUT_CSV}\n")
    ...
    async with async_playwright() as p:
        ...
        for channel in ["chrome", "msedge", None]:
            try:
                ...
                launch_args = {
                    "headless": config_hotel_gr.HEADLESS,
                    "slow_mo": config_hotel_gr.SLOW_MO,
                    "args": [
    ...
    print(f"[+] Results saved to: {config_hotel_gr.OUTPUT_CSV}")
