...
def main():
    ...
    print(f"[*] Downloading Portugal locations database from: {url}")
    ...
    for row in reader:
        ...
        try:
            lat = float(lat_str)
            lng = float(lng_str)
            # Portugal bounding box coordinates check (Mainland + Islands)
            if not (32.0 < lat < 43.0) or not (-32.0 < lng < -6.0):
                continue
            
            raw_locations.append({
                "state": state if state else "Portugal",
                "name": locality,
                "lat": lat,
                "lng": lng
            })
    ...
    output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "locations_pt.py")
    print(f"[*] Writing {len(clustered_locations)} clustered locations to {output_path}...")
    
    try:
        with open(output_path, "w", encoding="utf-8") as f:
            f.write("# Generated automatically by generate_locations_pt.py\n")
            f.write("# Covers all populated locations in Portugal using spatial clustering.\n\n")
            f.write("LOCATIONS = [\n")
        ...
        print("[SUCCESS] locations_pt.py has been successfully created!")
