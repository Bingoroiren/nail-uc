...
def get_scraped_urls():
    ...
    scraped_urls = set()
    if os.path.exists(config_barber_nz.OUTPUT_CSV):
        try:
            with open(config_barber_nz.OUTPUT_CSV, mode='r', encoding='utf-8-sig') as f:
...
def append_to_csv(row_dict):
    ...
    file_exists = os.path.isfile(config_barber_nz.OUTPUT_CSV)
    try:
        with open(config_barber_nz.OUTPUT_CSV, mode='a', encoding='utf-8-sig', newline='') as f:
...
PROGRESS_FILE = os.path.join(os.path.dirname(config_barber_nz.OUTPUT_CSV), "scraping_progress_barber_nz.json")
            ...
            with open(config_barber_nz.OUTPUT_CSV, mode='r', encoding='utf-8-sig') as f:
                ...
                for row in reader:
                    ...
                    if loc_name and state:
                        pair = (loc_name.strip().lower(), state.strip().lower(), config_barber_nz.KEYWORDS[0].lower())
                        if pair not in completed:
                            completed.add(pair)
                            completed_list.append([loc_name.strip(), state.strip(), config_barber_nz.KEYWORDS[0]])
...
async def scroll_feed(page, max_scrolls=20):
    ...
    feed_selector = config_barber_nz.SELECTORS["results_container"]
...
async def extract_details(page, url, search_query, loc_info):
    ...
    sel = config_barber_nz.SELECTORS
...
async def process_search(page, keyword, loc_info, scraped_urls):
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            await page.goto(search_url, timeout=config_barber_nz.TIMEOUT, wait_until="domcontentloaded")
    ...
    link_selector = config_barber_nz.SELECTORS["listing_link"]
    ...
    if listings_count > 0:
        ...
        try:
            ...
            for _ in range(15): # Try for 3 seconds (15 * 200ms)
                h1_locator = page.locator(config_barber_nz.SELECTORS["business_name"])
            ...
            addr_loc = page.locator(config_barber_nz.SELECTORS["address"])
            address = ""
            if await addr_loc.count() > 0:
                addr_label = await addr_loc.first.get_attribute("aria-label")
    ...
    for index, url in enumerate(urls):
        ...
        await asyncio.sleep(random.uniform(config_barber_nz.MIN_DELAY, config_barber_nz.MAX_DELAY))
...
async def main():
    ...
    keywords = config_barber_nz.KEYWORDS
    
    print(f"[+] Total target locations: {len(locs)}")
    print(f"[+] Search keywords: {keywords}")
    print(f"[+] Output CSV path: {config_barber_nz.OUTPUT_CSV}\n")
    ...
    async with async_playwright() as p:
        ...
        for channel in ["chrome", "msedge", None]:
            try:
                ...
                launch_args = {
                    "headless": config_barber_nz.HEADLESS,
                    "slow_mo": config_barber_nz.SLOW_MO,
                    "args": [
    ...
    print(f"[+] Results saved to: {config_barber_nz.OUTPUT_CSV}")
