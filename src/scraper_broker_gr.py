...
def get_scraped_urls():
    ...
    scraped_urls = set()
    if os.path.exists(config_broker_gr.OUTPUT_CSV):
        try:
            with open(config_broker_gr.OUTPUT_CSV, mode='r', encoding='utf-8-sig') as f:
...
def is_greece_address(address):
    """Verifies if an address is in Greece (Ελλάδα)."""
    if not address:
        return False
    addr_lower = address.lower()
    if "greece" in addr_lower or "ελλάδα" in addr_lower or "ελλας" in addr_lower or ", gr" in addr_lower:
        return True
    
    gr_cities = [
        "athens", "αθήνα", "athina", "thessaloniki", "θεσσαλονίκη", "patras", "πάτρα", 
        "heraklion", "ηράκλειο", "larissa", "λάρισα", "volos", "βόλος", "rhodes", "ρόδος", 
        "chania", "χανιά", "ioannina", "ιωάννινα", "chalkida", "χαλκίδα", "kavala", "καβάλα", 
        "serres", "σέρρες", "alexandroupoli", "αλεξανδρούπολη", "xanthi", "ξάνθη", "kerkyra", 
        "κέρκυρα", "corfu", "mykonos", "μύκονος", "santorini", "σαντορίνη", "thira", "θήρα",
        "kos", "κως", "zakynthos", "ζάκυνθος", "kalamata", "καλαμάτα"
    ]
    if any(city in addr_lower for city in gr_cities):
        return True
        
    # Greek postal codes are 5 digits (usually formatted as e.g. 104 31 or 546 21)
    if re.search(r'\b\d{3}\s?\d{2}\b', addr_lower):
        return True
...
def append_to_csv(row_dict):
    ...
    file_exists = os.path.isfile(config_broker_gr.OUTPUT_CSV)
    try:
        with open(config_broker_gr.OUTPUT_CSV, mode='a', encoding='utf-8-sig', newline='') as f:
...
PROGRESS_FILE = config_broker_gr.PROGRESS_FILE
            ...
            with open(config_broker_gr.OUTPUT_CSV, mode='r', encoding='utf-8-sig') as f:
                ...
                for row in reader:
                    ...
                    if loc_name and state:
                        pair = (loc_name.strip().lower(), state.strip().lower(), config_broker_gr.KEYWORDS[0].lower())
...
async def scroll_feed(page, max_scrolls=20):
    ...
    feed_selector = config_broker_gr.SELECTORS["results_container"]
    ...
    while scrolls < max_scrolls:
        ...
        if "reached the end of the list" in inner_text.lower() or "reached the end" in inner_text.lower() or "τέλος της λίστα" in inner_text.lower() or "τέλος της λίστας" in inner_text.lower():
...
def parse_greece_address(address):
    """Extracts city/suburb, region (state), and postal code from a Greek address."""
    if not address:
        return None, None, None
    addr = re.sub(r',\s*Greece\s*$', '', address, flags=re.IGNORECASE).strip()
    addr = re.sub(r',\s*Ελλάδα\s*$', '', addr, flags=re.IGNORECASE).strip()
    
    # Greek postal codes are 5 digits, e.g., 104 31 or 84600
    postcode_match = re.search(r'\b\d{3}\s?\d{2}\b', addr)
    if postcode_match:
...
async def extract_details(page, url, search_query, loc_info):
    ...
    sel = config_broker_gr.SELECTORS
    ...
    if await addr_loc.count() > 0:
        ...
        if addr_label:
            address = addr_label.replace("Address:", "").replace("Διεύθυνση:", "").strip()
        else:
            address = await addr_loc.first.inner_text()
            address = address.strip()
            
    if not is_greece_address(address):
        print(f"    [-] Skipping: Address '{address}' is not in Greece.")
        return None
    ...
    if coords:
        ...
        # Safe bounding box for Greece
        if not (34.0 < actual_lat < 42.5) or not (19.0 < actual_lng < 29.8):
            print(f"    [-] Skipping: Coordinates ({actual_lat}, {actual_lng}) are outside Greece.")
            return None
            
    actual_suburb, actual_state, postcode = parse_greece_address(address)
    if not actual_state:
    ...
    if await rating_loc.count() > 0:
        ...
        span_reviews = rating_loc.first.locator('span[aria-label*="classificação"], span[aria-label*="avaliação"], span[aria-label*="review"], span[aria-label*="κριτικές"], span[aria-label*="βαθμολογία"]')
    ...
    try:
        closed_loc = page.locator('span:has-text("Permanently closed"), span:has-text("Fechado permanentemente"), span:has-text("Κλειστό οριστικά")')
    ...
    # Strict filter: category must match one of the allowed Greek labor broker tags
    if category:
        cat_lower = category.lower().strip()
        if not any(tag in cat_lower for tag in config_broker_gr.ALLOWED_CATEGORIES):
            print(f"    [-] Skipping: Category '{category}' is not in allowed Greek broker tags.")
            return None
    else:
        print(f"    [-] Skipping: No category tag found.")
        return None
...
async def process_search(page, keyword, loc_info, scraped_urls):
    ...
    search_query = f"{keyword} in {loc_info['name']}, Greece"
    print(f"\n[+] Searching: '{search_query}'")
    
    query_encoded = urllib.parse.quote_plus(keyword)
    zoom = loc_info.get("zoom", 11)
    search_url = f"https://www.google.com/maps/search/{query_encoded}/@{loc_info['lat']},{loc_info['lng']},{zoom}z?hl=el"
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            await page.goto(search_url, timeout=config_broker_gr.TIMEOUT, wait_until="domcontentloaded")
    ...
    link_selector = config_broker_gr.SELECTORS["listing_link"]
    ...
    if listings_count > 0:
        ...
        try:
            ...
            for _ in range(15):
                h1_locator = page.locator(config_broker_gr.SELECTORS["business_name"])
            ...
            addr_loc = page.locator(config_broker_gr.SELECTORS["address"])
            address = ""
            if await addr_loc.count() > 0:
                addr_label = await addr_loc.first.get_attribute("aria-label")
                address = addr_label.replace("Address:", "").replace("Διεύθυνση:", "").strip() if addr_label else await addr_loc.first.inner_text()
                address = address.strip()
                
            if address and not is_greece_address(address):
                print(f"[-] First listing is outside Greece ('{address}'). Skipping location.")
                return True
    ...
    for index, url in enumerate(urls):
        ...
        await asyncio.sleep(random.uniform(config_broker_gr.MIN_DELAY, config_broker_gr.MAX_DELAY))
...
async def main():
    print("="*60)
    print("      GREECE LABOR BROKER GOOGLE MAPS SCRAPER")
    print("="*60)
    ...
    locs = locations_gr.get_locations()
    keywords = config_broker_gr.KEYWORDS
    ...
    print(f"[+] Output CSV path: {config_broker_gr.OUTPUT_CSV}\n")
    ...
    async with async_playwright() as p:
        ...
        for channel in ["chrome", "msedge", None]:
            try:
                ...
                launch_args = {
                    "headless": config_broker_gr.HEADLESS,
                    "slow_mo": config_broker_gr.SLOW_MO,
                    "args": [
    ...
    print(f"[+] Results saved to: {config_broker_gr.OUTPUT_CSV}")
