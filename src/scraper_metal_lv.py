...
def get_scraped_urls():
    ...
    scraped_urls = set()
    if os.path.exists(config_metal_lv.OUTPUT_CSV):
        try:
            with open(config_metal_lv.OUTPUT_CSV, mode='r', encoding='utf-8-sig') as f:
...
def is_latvia_address(address):
    """Verifies if an address is in Latvia."""
    if not address:
        return False
...
def append_to_csv(row_dict):
    ...
    file_exists = os.path.isfile(config_metal_lv.OUTPUT_CSV)
    try:
        with open(config_metal_lv.OUTPUT_CSV, mode='a', encoding='utf-8-sig', newline='') as f:
...
PROGRESS_FILE = config_metal_lv.PROGRESS_FILE
            ...
            with open(config_metal_lv.OUTPUT_CSV, mode='r', encoding='utf-8-sig') as f:
                ...
                for row in reader:
                    ...
                    if loc_name and state:
                        pair = (loc_name.strip().lower(), state.strip().lower(), config_metal_lv.KEYWORDS[0].lower())
...
async def scroll_feed(page, max_scrolls=20):
    ...
    feed_selector = config_metal_lv.SELECTORS["results_container"]
...
async def extract_details(page, url, search_query, loc_info):
    ...
    # Strict filter: category must match one of the allowed fisheries/seafood tags
    if category:
        cat_lower = category.lower().strip()
        if not any(tag in cat_lower for tag in config_metal_lv.ALLOWED_CATEGORIES):
            print(f"    [-] Skipping: Category '{category}' is not in allowed tags.")
            return None
...
async def process_search(page, keyword, loc_info, scraped_urls):
    ...
    zoom = loc_info.get("zoom", 10)
    search_url = f"https://www.google.com/maps/search/{query_encoded}/@{loc_info['lat']},{loc_info['lng']},{zoom}z?hl=lv"
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            await page.goto(search_url, timeout=config_metal_lv.TIMEOUT, wait_until="domcontentloaded")
    ...
    link_selector = config_metal_lv.SELECTORS["listing_link"]
    ...
    if listings_count > 0:
        ...
        try:
            ...
            for _ in range(15):
                h1_locator = page.locator(config_metal_lv.SELECTORS["business_name"])
            ...
            addr_loc = page.locator(config_metal_lv.SELECTORS["address"])
            address = ""
            if await addr_loc.count() > 0:
                addr_label = await addr_loc.first.get_attribute("aria-label")
            ...
            if address and not is_latvia_address(address):
    ...
    for index, url in enumerate(urls):
        ...
        await asyncio.sleep(random.uniform(config_metal_lv.MIN_DELAY, config_metal_lv.MAX_DELAY))
...
async def main():
    print("="*60)
    print("      LATVIA METALWORKING / METAL FABRICATION GOOGLE MAPS SCRAPER")
    print("="*60)
    ...
    keywords = config_metal_lv.KEYWORDS
    ...
    print(f"[+] Output CSV path: {config_metal_lv.OUTPUT_CSV}\n")
    ...
    async with async_playwright() as p:
        ...
        for channel in ["chrome", "msedge", None]:
            try:
                ...
                launch_args = {
                    "headless": config_metal_lv.HEADLESS,
                    "slow_mo": config_metal_lv.SLOW_MO,
                    "args": [
    ...
    print(f"[+] Results saved to: {config_metal_lv.OUTPUT_CSV}")
