...
INPUT_CSV = os.path.join(ROOT_DIR, "data", "raw", "farm_austria.csv")
OUTPUT_CSV = os.path.join(ROOT_DIR, "data", "formatted", "farm_austria_with_emails_formatted.csv")
CLEAN_DEDUP_CSV = os.path.join(ROOT_DIR, "data", "formatted", "farm_austria_clean_dedup.csv")
PROGRESS_FILE = os.path.join(ROOT_DIR, "data", "progress", "scraping_progress_farm_at_emails.json")
...
async def main():
    ...
    safe_print(f"[*] Processing Austria Farm listings from: {INPUT_CSV}")
    ...
    async with async_playwright() as p:
        ...
        for i, row in enumerate(formatted_rows):
            ...
            if is_valid_site:
                safe_print(f"[{i+1}/{total}] Crawling emails for Austria Farm: '{comp_name}' -> {web_url}...")
    ...
    safe_print(f"\n==================================================")
    safe_print(f"[SUCCESS] Austria Farm Data Processed & Formatted.")
