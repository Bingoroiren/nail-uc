...
CANDIDATES = [
    os.path.join(ROOT_DIR, "data", "formatted", "farm_ireland_with_emails.csv"),
    os.path.join(ROOT_DIR, "data", "raw", "farm_ireland.csv")
]
...
BUSINESS_PREFIXES = {
    ...
    'admin', 'office', 'hello', 'enquiries', 'farm'
}
...
# Category translations from English to Vietnamese for Farms & Agriculture
CATEGORY_TRANSLATIONS = {
    "farm": "Nông trại / Trang trại",
    "dairy farm": "Nông trại sữa / Trang trại chăn nuôi bò sữa",
    "honey farm": "Trang trại nuôi ong lấy mật",
    "fruit and vegetable wholesaler": "Nhà bán buôn hoa quả & Rau củ",
    "organic farm": "Nông trại hữu cơ (Organic Farm)",
    "orchard": "Vườn cây ăn quả",
    "dairy supplier": "Nhà cung cấp sản phẩm từ sữa",
    "fruit and vegetable processing": "Cơ sở chế biến hoa quả & Rau củ",
    "corporate office": "Văn phòng doanh nghiệp / Trụ sở chính",
    "manufacturer": "Nhà sản xuất",
    "mushroom farm": "Trang trại trồng nấm",
    "greenhouse": "Nhà kính trồng trọt (Greenhouse)",
    "agricultural producer": "Nhà sản xuất nông nghiệp",
    "farm produce market": "Chợ / Cửa hàng nông sản",
    "produce wholesaler": "Nhà bán buôn nông sản"
}

def translate_category(raw_cat):
    if not raw_cat:
        return "Nông nghiệp & Trang trại Ireland"
...
def main():
    ...
    for r in rows:
        ...
        best_email = clean_and_score_email(original_email)
        if not best_email:
            website = get_field(r, 'Website', 'Liên Hệ', 'URL', 'website')
            best_email = guess_email_from_website(website)
        r['Best_Email'] = best_email
