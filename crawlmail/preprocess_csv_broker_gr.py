...
INPUT_CSV = os.path.join(os.path.dirname(SCRIPT_DIR), "broker_greece.csv")
BACKUP_CSV = os.path.join(os.path.dirname(SCRIPT_DIR), "broker_greece_backup.csv")
...
def preprocess():
    ...
    for row in unique_rows:
        ...
        if not category or not any(tag in category for tag in ALLOWED_CATEGORIES):
            filtered_out_count += 1
            continue
        filtered_rows.append(row)
    unique_rows = filtered_rows
    print(f"[*] Filtered out {filtered_out_count} rows with irrelevant or missing categories. Rows remaining: {len(unique_rows)}.")
