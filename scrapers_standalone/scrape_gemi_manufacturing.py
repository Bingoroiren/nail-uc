...
OUTPUT_CSV = os.path.join(ROOT_DIR, "data", "raw", "gemi_manufacturing_companies.csv")
...
async def main():
    print("="*70)
    print("  GEMI GREECE - BỘ CÀO THÔNG TIN CÔNG TY SẢN XUẤT (MANUFACTURING)")
    print("="*70)
    ...
    async with async_playwright() as p:
        ...
        print(" 1. Click vào nút 'Φίλτρα' (Filters) trên giao diện web.")
        print(" 2. Tại ô 'Δραστηριότητα' (Activity), click chọn 'Γ ΜΕΤΑΠΟΙΗΣΗ'.")
            ...
            for idx, (name, href) in enumerate(companies_data, 1):
                ...
                with open(OUTPUT_CSV, mode="a", encoding="utf-8-sig", newline="") as f:
                    ...
                    writer.writerow({
                        ...
                        "Số GEMI": gemi,
                        "Điện thoại": f"'{phone}" if phone else "",
                        "Email": email,
